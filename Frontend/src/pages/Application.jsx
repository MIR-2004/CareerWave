      ...
      <div className='flex gap-2 mb-6 mt-3'>
        {
          ...
          <button onClick={updateResume} className='bg-gray-100 border border-green-400 rounded-lg px-4 py-2'>Save</button>
            ...
            <button onClick={() => setIsEdit(true)} className='text-gray-500 border-gray-300 rounded-lg px-4 py-2 cursor-pointer'>
              Edit
            </button>
          </div>)
        }
      ...
      <table className='min-w-full bg-white border rounded-lg'>
        <thead>
          <tr>
            ...
            <th className='py-3 px-4 border-b text-left '>Status
            </th>
          </tr>
        </thead>
        <tbody>
          {jobsApplied.map((job, index) => 
            true ? (
              <tr key={index}>
                ...
                <span className={`${job.status === 'Accepted' ? 'bg-green-100' : job.status === 'Rejected' ? 'bg-red-100' : 'bg-yellow-100'} px-4 py-1.5 rounded`}>
                {job.status}
              </span>
                </td>
              </tr>
